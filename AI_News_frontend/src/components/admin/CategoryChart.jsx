...
export default function CategoryChart({ distribution }) {
  ...
  return (
  <div className="bg-paper border border-border rounded-[2.5rem] p-10 h-[650px] flex flex-col relative overflow-hidden group shadow-sm transition-all duration-300">
    ...
    {/* Added min-h-0 to the flex child to prevent overflow issues in flexbox */}
    <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar scroll-smooth min-h-0">
      {/* 
          IMPORTANT: Added a logical fallback for chartData 
          to ensure height is always at least 400px 
      */}
      <div style={{ height: Math.max((chartData?.length || 0) * 50, 400), width: '100%' }}>
